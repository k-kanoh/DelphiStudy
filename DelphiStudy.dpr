...
begin
  ...
  hProc := OpenProcess($1000 { PROCESS_QUERY_LIMITED_INFORMATION }, False, PID);
  if hProc = 0 then
    Exit(False);
  try
...
begin
  ...
  begin
    ...
    begin
      LAltDown := False;
      if not AltDirty then begin SendF23; SetIme(False); end;
    end
    else if (VK = VK_RALT) and RAltDown then
    begin
      RAltDown := False;
      if not AltDirty then begin SendF23; SetIme(True); end;
    end;
  end;
end;
...
var
  Msg: TMsg;
begin
  Hook := SetWindowsHookEx(WH_KEYBOARD_LL, @HookCallback, GetModuleHandle(nil), 0);
